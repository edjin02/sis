...
var FullCalendar = (function (exports) {
    ...
    */
    ...
    function isInteractionPropsValid(state, context, dateSpanMeta, filterConfig) {
        ...
        var courseEventStore = interaction.mutatedEvents;
        var courseDefs = courseEventStore.defs;
        var courseInstances = courseEventStore.instances;
        var courseConfigs = compileEventUis(courseDefs, interaction.isEvent ?
            state.eventUiBases :
            { '': currentState.selectionConfig });
        if (filterConfig) {
            courseConfigs = mapHash(courseConfigs, filterConfig);
        }
        // exclude the course events. TODO: exclude defs too?
        var otherEventStore = excludeInstances(state.eventStore, interaction.affectedEvents.instances);
        var otherDefs = otherEventStore.defs;
        var otherInstances = otherEventStore.instances;
        var otherConfigs = compileEventUis(otherDefs, state.eventUiBases);
        for (var courseInstanceId in courseInstances) {
            var courseInstance = courseInstances[courseInstanceId];
            var courseRange = courseInstance.range;
            var courseConfig = courseConfigs[courseInstance.defId];
            var courseDef = courseDefs[courseInstance.defId];
            // constraint
            if (!allConstraintsPass(courseConfig.constraints, courseRange, otherEventStore, state.businessHours, context)) {
                return false;
            }
            // overlap
            ...
            for (var otherInstanceId in otherInstances) {
                ...
                // intersect! evaluate
                if (rangesIntersect(courseRange, otherInstance.range)) {
                    var otherOverlap = otherConfigs[otherInstance.defId].overlap;
                    // consider the other event's overlap. only do this if the course event is a "real" event
                    if (otherOverlap === false && interaction.isEvent) {
                        return false;
                    }
                    if (courseConfig.overlap === false) {
                        return false;
                    }
                    if (eventOverlapFunc && !eventOverlapFunc(new EventApi(context, otherDefs[otherInstance.defId], otherInstance), // still event
                    new EventApi(context, courseDef, courseInstance))) {
                        return false;
                    }
                }
            }
            ...
            for (var _i = 0, _a = courseConfig.allows; _i < _a.length; _i++) {
                var courseAllow = _a[_i];
                var courseDateSpan = __assign(__assign({}, dateSpanMeta), { range: courseInstance.range, allDay: courseDef.allDay });
                var origDef = calendarEventStore.defs[courseDef.defId];
                var origInstance = calendarEventStore.instances[courseInstanceId];
                var eventApi = void 0;
                if (origDef) { // was previously in the calendar
                    eventApi = new EventApi(context, origDef, origInstance);
                }
                else { // was an external event
                    eventApi = new EventApi(context, courseDef); // no instance, because had no dates
                }
                if (!courseAllow(buildDateSpanApiWithContext(courseDateSpan, context), eventApi)) {
                    return false;
                }
            }
        }
    ...
    // Constraint Utils
    // ------------------------------------------------------------------------------------------------------------------------
    function allConstraintsPass(constraints, courseRange, otherEventStore, businessHoursUnexpanded, context) {
        for (var _i = 0, constraints_1 = constraints; _i < constraints_1.length; _i++) {
            var constraint = constraints_1[_i];
            if (!anyRangesContainRange(constraintToRanges(constraint, courseRange, otherEventStore, businessHoursUnexpanded, context), courseRange)) {
                return false;
            }
        }
        return true;
    }
    function constraintToRanges(constraint, courseRange, // for expanding a recurring constraint, or expanding business hours
    otherEventStore, // for if constraint is an even group ID
    businessHoursUnexpanded, // for if constraint is 'businessHours'
    context) {
        if (constraint === 'businessHours') {
            return eventStoreToRanges(expandRecurring(businessHoursUnexpanded, courseRange, context));
        }
        if (typeof constraint === 'string') { // an group ID
            return eventStoreToRanges(filterEventStoreDefs(otherEventStore, function (eventDef) { return eventDef.groupId === constraint; }));
        }
        if (typeof constraint === 'object' && constraint) { // non-null object
            return eventStoreToRanges(expandRecurring(constraint, courseRange, context));
        }
    ...
    */
    var PointerDragging = /** @class */ (function () {
        function PointerDragging(containerEl) {
            ...
            this.selector = ''; // will cause courseEl in all emitted events to be this element
            this.handleSelector = '';
        ...
        PointerDragging.prototype.tryStart = function (ev) {
            var courseEl = this.querycourseEl(ev);
        ...
        PointerDragging.prototype.querycourseEl = function (ev) {
            if (this.selector) {
            ...
            _this.onPointerDown = function (ev) {
                if (!_this.isDragging) { // so new drag doesn't happen while revert animation is going
                    ...
                    if (_this.isInteracting && // not destroyed via pointerdown handler
                        ...
                        _this.mirror.start(ev.courseEl, ev.pageX, ev.pageY); // must happen on first pointer down
                        _this.startDelay(ev);
    ...
    */
    var HitDragging = /** @class */ (function () {
        ...
        HitDragging.prototype.processFirstCoord = function (ev) {
            ...
            var courseEl = ev.courseEl;
            var courseRect;
            if (courseEl !== document) {
                courseRect = computeRect(courseEl);
                adjustedPoint = constrainPoint(adjustedPoint, courseRect);
            }
            var initialHit = this.initialHit = this.queryHitForOffset(adjustedPoint.left, adjustedPoint.top);
            if (initialHit) {
                if (this.usecourseCenter && courseRect) {
                    var slicedcourseRect = intersectRects(courseRect, initialHit.rect);
                    if (slicedcourseRect) {
                        adjustedPoint = getRectCenter(slicedcourseRect);
                    }
                }
    ...
    */
    ...
    var EventDragging = /** @class */ (function (_super) {
        ...
        function EventDragging(settings) {
            ...
            // internal state
            _this.courseEl = null;
            _this.courseSeg = null; // the seg being selected/dragged
            ...
            _this.handlePointerDown = function (ev) {
                ...
                _this.courseEl = ev.courseEl;
                var courseSeg = _this.courseSeg = getElSeg(ev.courseEl);
                var eventRange = _this.eventRange = courseSeg.eventRange;
                ...
                _this.isDragging = isValid &&
                    ev.courseEl.classList.contains('fc-event-draggable');
            };
            _this.handleDragStart = function (ev) {
                ...
                if (_this.isDragging) {
                    ...
                    if (validMutation) {
                        ...
                        if (receivingContext_1 === initialContext_1) {
                            ...
                            initialContext_1.emitter.trigger('eventDrop', __assign(__assign(__assign({}, eventChangeArg), transformed), { el: ev.courseEl, delta: validMutation.datesDelta, jsEvent: ev.origEvent, view: initialView }));
                            ...
                            initialContext_1.emitter.trigger('eventLeave', __assign(__assign({}, eventRemoveArg), { draggedEl: ev.courseEl, view: initialView }));
                            initialContext_1.dispatch({
                                type: 'REMOVE_EVENTS',
                                eventStore: relevantEvents_1,
                            });
                            ...
                            receivingContext_1.emitter.trigger('drop', __assign(__assign({}, buildDatePointApiWithContext(finalHit.dateSpan, receivingContext_1)), { draggedEl: ev.courseEl, jsEvent: ev.origEvent, view: finalHit.component.context.viewApi }));
                            receivingContext_1.emitter.trigger('eventReceive', __assign(__assign({}, eventAddArg), { draggedEl: ev.courseEl, view: finalHit.component.context.viewApi }));
                        }
                    }
                    else {
            ...
            hitDragging.usecourseCenter = settings.useEventCenter;
            hitDragging.emitter.on('pointerdown', _this.handlePointerDown);
            hitDragging.emitter.on('dragstart', _this.handleDragStart);
            hitDragging.emitter.on('hitupdate', _this.handleHitUpdate);
            hitDragging.emitter.on('pointerup', _this.handlePointerUp);
    ...
    var EventResizing = /** @class */ (function (_super) {
        ...
        function EventResizing(settings) {
            ...
            _this.draggingSeg = null; // TODO: rename to resizingSeg? courseSeg?
            _this.eventRange = null;
            ...
            _this.handleHitUpdate = function (hit, isFinal, ev) {
                ...
                if (hit) {
                    mutation = computeMutation(initialHit, hit, ev.courseEl.classList.contains('fc-event-resizer-start'), eventInstance.range, context.pluginHooks.eventResizeJoinTransforms);
                }
                if (mutation) {
        ...
        EventResizing.prototype.querySegEl = function (ev) {
            return elementClosest(ev.courseEl, '.fc-event');
        };
    ...
    */
    var ExternalElementDragging = /** @class */ (function () {
        function ExternalElementDragging(dragging, suppliedDragMeta) {
            ...
            this.dragMeta = null;
            this.handleDragStart = function (ev) {
                _this.dragMeta = _this.buildDragMeta(ev.courseEl);
            };
            this.handleHitUpdate = function (hit, isFinal, ev) {
                ...
                if (hit) {
                    ...
                    if (_this.canDropElOnCalendar(ev.courseEl, receivingContext)) {
            ...
            this.handleDragEnd = function (pev) {
                ...
                if (receivingContext && droppableEvent) {
                    ...
                    receivingContext.emitter.trigger('drop', __assign(__assign({}, buildDatePointApiWithContext(finalHit.dateSpan, receivingContext)), { draggedEl: pev.courseEl, jsEvent: pev.origEvent, view: finalView }));
                    if (dragMeta.create) {
        ...
        ExternalElementDragging.prototype.buildDragMeta = function (courseEl) {
            if (typeof this.suppliedDragMeta === 'object') {
                return parseDragMeta(this.suppliedDragMeta);
            }
            if (typeof this.suppliedDragMeta === 'function') {
                return parseDragMeta(this.suppliedDragMeta(courseEl));
            }
            return getDragMetaFromEl(courseEl);
        };
        ...
        function ExternalDraggable(el, settings) {
            ...
            this.handleDragStart = function (ev) {
                if (ev.isTouch &&
                    ...
                    ev.courseEl.classList.contains('fc-event')) {
